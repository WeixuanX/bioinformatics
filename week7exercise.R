...
gofstat(fit_pois)
##换个模型用负二项分布fit a nbinom to the data instead
...
Zi_nb1_ar1_mod <- glmmTMB(abundance ~ scale(standardised_time) * scale(n.threats) + (1|population) + ar1(factor(scale(standardised_time))-1|population), 
                          ...
                          control=glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))
##2.11##load DHARMa
##前面install了就不需要了：install.packages("DHARMa")
library(DHARMa)
...
##plot out the residuals
plot(Zi_nb1_mod_sim)
##2.12## test to see where there are outliers, in our case not significant so we dont need to worry
...
##2.14plot the predicted against the observed
library(predict)
ggplot(species_single0, aes(x = abundance, 
  ...
  xlab("Observed") +
  ylab("Predicted")
##2.16summarise the model 
summary(Zi_nb1_mod)


##2.17 make a blank data set which includes the variables in the model
